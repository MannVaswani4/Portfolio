      ...
      <div className="max-w-[1200px] mx-auto space-y-12 relative z-10">
        ...
        {project.coverImage && (
          <motion.div
            ...
            className={`w-full ${project.id === 'accredian' || project.id === 'humanness' || project.id === 'globalair' ? 'aspect-[16/10] md:aspect-[16/9]' : 'aspect-[21/9]'} rounded-[2rem] overflow-hidden border border-outline-variant/15 relative bg-[#0e0e0e] shadow-[0_12px_40px_rgba(0,0,0,0.5)]`}
          >
            <img
              src={project.coverImage}
              alt={project.title}
              className={`w-full h-full object-cover ${project.id === 'agrilo' || project.id === 'accredian' || project.id === 'humanness' || project.id === 'globalair' ? 'object-top' : 'object-center'}`}
            />
